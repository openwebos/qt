...
QGoodLifeCursorDraw::QGoodLifeCursorDraw()
{
	KADP_InitSystem();
	
    m_cursorImgPath = QDir::homePath() + "/res/lglib/cursorTypeAszSstN.png";
...
void QGoodLifeCursorDraw::setConfigFBDev()
{
...
int QGoodLifeCursorDraw::Cursor_LoadPNG(CURSOR_FILE_INFO_T *pCursorFileList)
{
	long 		fdPng;
	struct stat sbPng;
	size_t		nBytes;
...
void QGoodLifeCursorDraw::initGfx()
{
	KADP_GFX_Open();
	...
	gCursorWidthScale = (float)GOODLIFE_SCREEN_WIDTH / GOODLIFE_MOUSE_WIDTH;
	gCursorHeightScale = (float)GOODLIFE_SCREEN_HEIGHT / GOODLIFE_MOUSE_HEIGHT;
...
void QGoodLifeCursorDraw::initFBDev()
{
    KADP_FBDEV_GetSurface(POSD_CURSOR_ID, 0, &m_FBSurface[0]);
    KADP_FBDEV_GetSurface(POSD_CURSOR_ID, 1, &m_FBSurface[1]);
}

bool QGoodLifeCursorDraw::createCursorSurface(LX_GFX_SURFACE_T **ppSurface, 
                                            unsigned int width,
                                            unsigned int height,
                                            signed int bitDepth,
...
bool QGoodLifeCursorDraw::setBackgroundColor(unsigned int x,
                                           unsigned int y,
                                           unsigned int w,
                                           unsigned int h,
                                           unsigned int color)
...
bool QGoodLifeCursorDraw::update()
{
...
int QGoodLifeCursorDraw::setPosition(uint x, uint y, uint w, uint h,
...
int QGoodLifeCursorDraw::setVisible(bool visible)
{
	KADP_FBDEV_SetVisible(POSD_CURSOR_ID, visible);
	return 0;
}

int QGoodLifeCursorDraw::setClearFBDev(void)
{
	long drawIdx;
	LX_RECT_T rect;
...
int QGoodLifeCursorDraw::setCursorImg(int index)
{
	LX_RECT_T 	srcRect;
	LX_RECT_T 	dstRect;
	int 		drwIdx;
